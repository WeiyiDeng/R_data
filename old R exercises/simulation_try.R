## from R programming course by JHU in coursa, Lecture 4 Simulation, also see slides 
...
#-----try simulation of binary y---------------??
set.seed(401716)
x <- rnorm(100)
w <- -1 + 2*x
p <- exp(w)/(1+exp(w))
y <- ifelse(p<0.5,0,1)                     # use ifelse to create binary variable
y
#-----regression not useful illustration--------
reg <- lm(y~x)
plot(x,y)
